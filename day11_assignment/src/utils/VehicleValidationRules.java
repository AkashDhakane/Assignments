...
//		System.out.println("no dup detected!!!!");
	...
	public static Vehicle findByChasisNo(int chasisNo, ArrayList<Vehicle> vehicles) throws VehicleHandlingException
	{
		...
		int indx = vehicles.indexOf(v1);
		if(indx==-1)
				throw new VehicleHandlingException("Invalid chasis no!!!!!");
		return vehicles.get(indx);	
			
	}
	
	//aupdate
		public static void updatePrice(int chasisNo, double price ,ArrayList<Vehicle> vehicles) throws VehicleHandlingException
		{
			Vehicle v1=new Vehicle(chasisNo);
			int indx = vehicles.indexOf(v1);
			if(indx==-1)
					throw new VehicleHandlingException("Invalid chasis no!!!!!");
			vehicles.get(indx).setPrice(price);
			System.out.println("price updated");
				
		}
		
		//delete
		public static void deleteVehicle(int chasisNo,ArrayList<Vehicle> vehicles) throws VehicleHandlingException
		{
			Vehicle v1=new Vehicle(chasisNo);
			if(vehicles.remove(v1))
				System.out.println("Vehicle removed");
		}
		
		//apply vehicle details
		public static void applyDiscount(String date,String category,double discount,ArrayList<Vehicle> vehicles) throws ParseException {
			Date parsDate = Vehicle.sdf.parse(date);
			Category parsCategory = validateCategory(category);
			for(Vehicle v:vehicles) {
				if(v.getManufactureDate().before(parsDate) && v.getCategory().equals(parsCategory))
					v.setPrice(v.getPrice()-(v.getPrice()*(discount/100)));
			}
			System.out.println("Discount applied on all valid vehicles ");
		}
		
		//add display by city
		public static void vehicleByCity(String city,ArrayList<Vehicle> vehicles) {
			for(Vehicle v:vehicles) {
				if(v.getCity().equals(city))
					System.out.println("Chasi no = "+v.getChasisNo() + "\nprice = "+v.getPrice());
			}
		}
